          ...
          for ia in xrange(2):
            for ib in xrange(2):
              ...
              for branch in xrange(0, branches):
                ...
                if r:
                  match = False
                  for pkey in laws_jacobian_weierstrass:
                    assumeLaw, require = laws_jacobian_weierstrass[pkey](A, B, pa, pb, pA, pB, pC)
...
def check_symbolic_jacobian_weierstrass(name, A, B, branches, formula):
  ...
  for branch in xrange(branches):
    ...
    for pkey in laws_jacobian_weierstrass:
      res[pkey].append((check_symbolic_function(R, assumeFormula, assumeBranch, laws_jacobian_weierstrass[pkey], A, B, pa, pb, pA, pB, pC), branch))
